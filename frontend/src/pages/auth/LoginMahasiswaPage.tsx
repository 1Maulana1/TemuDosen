/**
 * LoginMahasiswaPage — /login/mahasiswa
 *
 * Role-specific login page for students. Field is Email + Password (backend
 * only supports email-based auth via POST /api/auth/login/ — NIM login isn't
 * possible, no lookup endpoint exists; decided during audit).
 *
 * Design and auth flow copied from LoginPage.tsx (not reinvented): same
 * gradient background, card, CTA styling, and login()/getRoleRedirect() logic.
 * Only the title and cross-role navigation links differ.
 */
...
// This page only accepts 'student' accounts. Actual role values confirmed
// against the User type in api/auth.ts — do not hardcode without checking.
const EXPECTED_ROLE: User['role'] = 'student';
    ...
    <div className="min-h-screen bg-[image:var(--gradient-auth)] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="bg-surface rounded-2xl shadow-2xl border border-neutral-gray/20 p-10">
          ...
          <p className="text-center text-sm font-bold text-slate-800 mb-8">Login Mahasiswa</p>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            {/* Role mismatch — account exists but isn't a mahasiswa account */}
            {roleMismatch && (
              <div
                role="alert"
                aria-live="polite"
              ...
              >
                <p>Akun ini bukan akun mahasiswa. Silakan login di halaman yang sesuai.</p>
                <Link
