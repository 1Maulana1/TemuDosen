...
**Success Criteria** (what must be TRUE):
  ...
  2. Ketua Jurusan can view a digitized guidance history (timestamps, durations, symptoms, approved summaries) across all lecturers — 🟡 PARTIAL: history/timestamps/durations/symptoms all present and tested (`KetuaJurusanStatsView`/`KetuaJurusanExportView`); "approved summaries" column is confirmed **still empty as of 2026-07-06** — Phase 6's `SessionLogbook` now exists, but `KetuaJurusanStatsView` was never updated to reference it, so the summary data has nowhere to surface
