...
**Status**: PARTIAL — SC1 (ADVICE-01) UI built 2026-07-06; SC2-6 unchanged. See `phases/07-advisory-continuity-campus-logbook/07-VERIFICATION.md` (2026-07-03) for the original backend-only verification; this ROADMAP entry has been updated past it for SC1.
**Success Criteria** (what must be TRUE):
  ...
  2. Lecturer can view the complete advice history and follow-up status for each advisee student — ❌ STILL NOT SATISFIED: what was built above is per-session only (advice items scoped to one `LecturerSessionDetail` page), not an aggregate cross-session view of one advisee's full advice history. The only aggregate view (`KetuaJurusanComplianceView`) remains ketua jurusan/admin-only. A dedicated lecturer-facing "this student's advice history across all their sessions" view/endpoint still doesn't exist
