...
> **Provenance note.** `ActionItem` model + `SessionActionItemsView`/`CompleteActionItemView`/`KaprodiComplianceView` were implemented in Farel's Phase 2 branch merge (`4e077a0`), alongside `KaprodiDashboard.tsx`'s compliance-rate *display*. No formal GSD plan/verification loop was run, and no test coverage existed before this pass — `test_action_items.py` (14 tests) is new.
...
| 4 | If the campus API is unavailable or unconfigured, system offers a CSV/PDF export of the summary for manual upload | ❌ NOT STARTED (for summaries specifically) | `KaprodiExportView` (verified in `08-VERIFICATION.md`) already does CSV+PDF export of guidance-history data (FR-KP03), which is adjacent but not the same thing — it exports session/workload data, not "the summary" (which doesn't exist) |
...
No bugs found in the backend logic itself — `SessionActionItemsView`, `CompleteActionItemView`, and `KaprodiComplianceView` all behave correctly against their own scope (ownership checks, permission checks, empty-state handling, compliance-rate math). The gap here is entirely about **what doesn't exist yet** (UI, campus sync), not incorrect code.
...
| ADVICE-02 | Lecturer views advice history per advisee | NOT SATISFIED | No lecturer-scoped aggregate view exists (only kaprodi/admin, and only per-session for the lecturer) |
