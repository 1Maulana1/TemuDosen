...
| 06-06-T3 | 06-06 | 4 | STT-04 | T-06-15, T-06-16 | Approve button disabled until transcript expanded once; "Perlu Verifikasi" chip always-visible + top-sorted; permanence modal | build/lint only — behavioral proof now lives in 06-06-T4 | `npm run build && npm run lint` | ✅ | ✅ green for the build half (clean) — 2026-07-05. `npm run lint` **does not exist**: no `lint` script in `package.json`, no ESLint config or dependency anywhere in the project. This is a real gap in the plan's verify command, not an environment issue; `npm run build`'s `tsc -b` type-check is the only available substitute. All 4 acceptance-criteria greps pass. **✅ RESOLVED (2026-07-05, explicit user decision after 06-06-T5 went green):** was flagged **⛔ OPEN GAP — BLOCKS PHASE 6 SIGN-OFF** — STT-04 edit/reject capability missing; a lecturer facing one wrong AI point could only approve the error permanently or stall the logbook forever, with no reject path connected to the existing manual-notes fallback. Gate Option A (06-06-T5) closed the dead-end: `RejectLogbookView` flips `ready_for_review`→`FAILED`, which is the exact state `ManualNotesView` already gates on — proven connected, not assumed, by `test_reject_then_manual_notes_endpoint_accepts_it` (backend 281/281, frontend 56/56, both green). The dangerous half of the gap (no way out except a permanent error) is closed. **Full per-item edit-ability (STT-04's complete "review/edit/approve" wording, Option B) remains an explicit, separate follow-up — NOT a sign-off blocker.** The backend is already ready for it (`ApproveLogbookSerializer` already accepts arbitrary `summary_edited` JSON, built in 06-04); only the frontend inline-edit UI (pencil-edit/trash-delete-with-undo) is outstanding, deferred by user decision, not discovered-and-ignored. |
...
| 06-06-T5 | 06-06 | 4 | STT-04 | T-06-24 | Added 2026-07-05 (Gate Option A) to close the 06-06-T3 OPEN GAP: `RejectLogbookView` (404→403→400-guard-on-`ready_for_review`→`FAILED`+`SystemLog(LOGBOOK_REJECTED)`) + `rejectLogbook()`/`RejectLogbookModal`/"Tolak Ringkasan" wiring; connection-proof that reject→FAILED triggers the *existing* `ManualNotesView` | integration (backend) + unit (Vitest/RTL, component behavior) | `pytest apps/logbook/tests/test_logbook_views.py -q -k reject` (backend) + `cd frontend && npm run build && npm run test -- --run LecturerLogbookReview` (frontend) | ✅ | ✅ green — 2026-07-05. Backend: 4/4 reject tests pass individually confirmed by name, including the connection-proof test (`test_reject_then_manual_notes_endpoint_accepts_it` — reject then POST manual-notes on the same logbook returns 200 not 400, final status=approved+is_manual=True, proving the transition is mechanically connected, not asserted in isolation). `grep -c LOGBOOK_REJECTED apps/logbook/tasks.py` = 0, `apps/logbook/views.py` = 1 (event_type confirmed lecturer-only, never pipeline-generated — closes T-06-24). Full backend suite 281/281 (277 baseline + 4 new). Frontend: build clean, 2 new behavioral tests pass (`reject_button_opens_confirmation_modal`, `confirm_reject_calls_api_and_navigates_to_manual_notes` — both individually confirmed by name), `aria-modal` grep on `RejectLogbookModal.tsx` = 1. Full frontend suite 56/56 (54 baseline + 2 new). **This closes the mechanism for 06-06-T3's OPEN GAP but does NOT itself downgrade that row — 06-06-T3 stays ⛔ OPEN GAP until the user explicitly authorizes the downgrade.** |
...
| 06-08-T1 | 06-08 | 5 | VIDEO-01 | T-06-19, T-06-20 | JitsiVideoProvider mounts with correct roomName/domain; loading→ready via mocked onApiReady; no dual-audio mixing code | unit (Vitest/RTL, jsdom mock) | `npm run test -- --run JitsiVideoProvider` | ✅ | ✅ green — 2026-07-05, 4/4 tests pass (mount+roomName/domain, loading state, loading→ready transition, labeled group role). `@jitsi/react-sdk@1.4.4` installed exact-version, 0 vulnerabilities. `prejoinPageEnabled`/`meet.jit.si` greps pass; dual-audio-mixing grep confirms 0 matches |
| 06-08-T2 | 06-08 | 5 | VIDEO-01, VIDEO-02 | T-06-21 | Both active-session cards embed JitsiVideoProvider; recording-path exact-count invariant holds (useMediaRecorder x2 / recorder.isRecording x1 / "Merekam" x1 in LecturerDashboard.tsx, baseline verified 2026-07-05); no apps/logbook file touched | build + full frontend regression + exact-count invariant | `cd frontend && npm run build && npm run test -- --run && grep -c "useMediaRecorder" src/pages/lecturer/LecturerDashboard.tsx \| grep -qx "2" && grep -c "recorder.isRecording" src/pages/lecturer/LecturerDashboard.tsx \| grep -qx "1" && grep -c "Merekam" src/pages/lecturer/LecturerDashboard.tsx \| grep -qx "1" && echo "RECORDING_SYMBOLS_INTACT"` | ✅ | ✅ green — 2026-07-05, build clean, full suite 54/54, `RECORDING_SYMBOLS_INTACT` re-confirmed AFTER all Task 2 edits (not just at the insertion point, per user request). Both cards embed (`VideoProvider` grep = 2 in each file). D-14 boundary verified: 0 `backend/apps/logbook/` files in the diff. `LecturerRequests.tsx` intentionally not touched — verified it has no active-session join-link affordance at all (only a meeting-link input field during approval); the plan's "equivalent" reference doesn't correspond to real code there. `LecturerQueue.tsx`/`StudentQueue.tsx` confirmed to carry zero recording symbols (nothing to protect, mic capture is lecturer-dashboard-only) |
