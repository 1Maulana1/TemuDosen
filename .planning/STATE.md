...
- **BLOCKER for Wave 4 (06-06/06-07) and Wave 5 (06-08)**: `frontend/node_modules` is not installed in this environment (confirmed 2026-07-05 — `npm run test` fails with `'vitest' is not recognized`). Waves 0-3 (06-01 through 06-05) are backend-only and unaffected, but 06-06/06-07's and 06-08's own `<verify>` blocks require `npm run build` / `npm run test -- --run <component>` to actually execute. Must run `npm install` in `frontend/` before starting Wave 4 execution, or those tasks' acceptance criteria cannot be checked (build/lint/test commands will error on missing binaries, not report real pass/fail).
